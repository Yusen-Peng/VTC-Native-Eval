...
def profile_model(model_name, batch_size=1, profiler='torch', device="cuda"):
    ...
    model = open_clip_local.create_model(model_name, force_custom_text=True, pretrained_hf=False)
    model.eval()
    ...
    model_cfg = open_clip_local.get_model_config(model_name)
    if model_cfg:
        vision_cfg = open_clip_local.CLIPVisionCfg(**model_cfg['vision_cfg'])
        text_cfg = open_clip_local.CLIPTextCfg(**model_cfg['text_cfg'])
...
def main():
    ...
    if args.model == 'all':
        parsed_model = open_clip_local.list_models()
    else:
