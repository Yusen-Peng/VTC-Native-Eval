...
def add_and_scale(AC: torch.Tensor, BD: torch.Tensor, scale: float) -> torch.Tensor:
    # AC, BD: [B, H, Q, K]
    return (AC + BD) * scale


class XLAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0.0, proj_drop=0.0, keep_rate=1.0,
        max_len=1024, # max token length you expect (N)
        pre_lnorm=True, # Transformer-XL default often True
    ):
        super().__init__()
        ...
        # Transformer-XL style projections
        self.qkv_net = nn.Linear(dim, 3 * num_heads * self.head_dim, bias=qkv_bias)
        self.r_net   = nn.Linear(dim, num_heads * self.head_dim, bias=False)
        self.o_net = nn.Linear(num_heads * self.head_dim, dim, bias=True)
        self.drop = nn.Dropout(proj_drop)
        self.dropatt = nn.Dropout(attn_drop)
        self.layer_norm = nn.LayerNorm(dim)

        # Transformer-XL learned per-head biases
        ...
        # ---- Relative position embedding table (1D) ----
        # We need relative distances in [-max_len+1, +max_len-1] => 2*max_len - 1
    ...
    def _rel_shift(x: torch.Tensor) -> torch.Tensor:
        # add a dummy column on last dim
    ...
    def _build_r(self, klen: int, device: torch.device) -> torch.Tensor:
        if klen > self.max_len:
            raise ValueError(f"klen={klen} exceeds max_len={self.max_len}. Increase max_len.")
        dist = torch.arange(klen - 1, -1, -1, device=device)  # [klen]
        idx = (self.max_len - 1) + dist
        r: torch.Tensor = self.rel_emb(idx)  # [klen, dim]
        return r.unsqueeze(1)  # [klen, 1, dim]

    def forward(self, x: torch.Tensor, keep_rate=None, tokens=None, attn_mask=None):
        if keep_rate is None:
            keep_rate = self.keep_rate
        ...
        # build r: (klen, 1, C)
        r = self._build_r(klen, x.device)

        # switch to (T, B, C) for Transformer-XL math
        w = x.transpose(0, 1)  # (N, B, C)

        if self.pre_lnorm:
            w_in = self.layer_norm(w)
        else:
            w_in = w

        w_heads = self.qkv_net(w_in)  # (N, B, 3*H*Dh)
        w_head_q, w_head_k, w_head_v = torch.chunk(w_heads, 3, dim=-1)
        r_head_k = self.r_net(r)  # (N, 1, H*Dh)

        # reshape
        w_head_q = w_head_q.view(qlen, B, self.num_heads, self.head_dim)
        w_head_k = w_head_k.view(klen, B, self.num_heads, self.head_dim)
        w_head_v = w_head_v.view(klen, B, self.num_heads, self.head_dim)
        r_head_k = r_head_k.view(klen, 1, self.num_heads, self.head_dim).squeeze(1)  # (klen, H, Dh)

        # attention scores: AC + BD
        # AC: content-based
        rw_head_q = w_head_q + self.r_w_bias  # broadcast (H,Dh) over (Q,B,H,Dh)
        AC = torch.einsum("ibnd,jbnd->bnij", rw_head_q, w_head_k)   # (B,H,Q,K)

        # BD: relative position
        rr_head_q = w_head_q + self.r_r_bias
        BD = torch.einsum("ibnd,jnd->bnij", rr_head_q, r_head_k)    # (B,H,Q,K)
        BD = self._rel_shift(BD)

        attn_score = add_and_scale(AC, BD, self.scale)              # (B,H,Q,K)

        if attn_mask is not None:
            # attn_mask expected (Q,K) or (B,Q,K) with True = mask
            if attn_mask.dim() == 2:
                attn_score.masked_fill_(attn_mask[None, None, :, :], -float("inf"))
            elif attn_mask.dim() == 3:
                attn_score.masked_fill_(attn_mask[:, None, :, :], -float("inf"))

        attn_prob = F.softmax(attn_score, dim=-1)                   # (B,H,Q,K)
        attn_prob = self.dropatt(attn_prob)

        # attention output
        attn_vec = torch.einsum("bnij,jbnd->ibnd", attn_prob, w_head_v)  # (Q,B,H,Dh)
        attn_vec = attn_vec.contiguous().view(qlen, B, self.num_heads * self.head_dim)
        attn_out = self.o_net(attn_vec)                              # (Q,B,C)
        ...
        x_out = out.transpose(0, 1)  # back to (B,N,C)
        ...
        if (self.keep_rate < 1 and keep_rate < 1) or (tokens is not None):
            ...
            if tokens is not None:
                left_tokens = tokens
            if left_tokens == N - 1:
                return x_out, None, None, None, left_tokens
            assert left_tokens >= 1

            # CLS attention to other tokens:
            # attn_prob: (B,H,Q,K); cls query is q=0, exclude k=0
            cls_attn = attn_prob[:, :, 0, 1:]         # (B,H,N-1)
            cls_attn = cls_attn.mean(dim=1)           # (B,N-1)

            _, idx = torch.topk(cls_attn, left_tokens, dim=1, largest=True, sorted=True)  # (B,left_tokens)
            index = idx.unsqueeze(-1).expand(-1, -1, C)  # (B,left_tokens,C)

            return x_out, index, idx, cls_attn, left_tokens
...
class XLBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = XLAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias,
                              attn_drop=attn_drop, proj_drop=drop, keep_rate=keep_rate)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
...
class EViT_XL_adapted(nn.Module):
    """ EViT XL adapted"""
